...
class KeyStoreService{
    /**
     通过keyStore和密码获取私钥
    **/
   static func getprivateKey(keysData:Data,password:String) -> String {
        
        //验证mac是否一致
        if !verifyPassword(keyData: keysData, passWord: password) {
             return ""
        }
        ...
        if let salt = json["kdfparams"]["salt"].string{
            ...
            let derivedKey = ArgonManage.argon2id(password: password, salt: salts)
    ...
    /**
     通过私钥获取公钥
    **/
  static  func privateKeyToPublicKey(privateKey:String) -> String {
    ...
    **/
 static   func verifyPassword(keyData:Data,passWord:String) ->Bool {
       
        let json=JSON(keyData)
        if let salt = json["kdfparams"]["salt"].string{
           // print("salt:====\(salt)")
            let salts = String(data: Data(hexString:salt)!, encoding: String.Encoding.utf8)!
            //print("salts:====\(salts)")
            let derivedKey = ArgonManage.argon2id(password: passWord, salt: salts) //e77c1e3ab4aa49a778f564c099d1cec68692a4752f62bb1584d713f544c5489c
